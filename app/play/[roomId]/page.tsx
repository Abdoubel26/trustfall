...
export default function GameRoom() {
  ...
  // Dynamic ambient classes based on outcome
  const ambientBg =
    outcome === "dopamine"
      ? "bg-amber-950/50"
      : outcome === "shame"
  ...
  return (
    <div
      className={`min-h-screen flex flex-col items-center justify-between p-4 md:p-6 select-none font-mono transition-all duration-1000 relative overflow-hidden ${ambientBg}`}
    >
      {/* Background layer - swap the gif path when you find a good one */}
      ...
      <div className={`relative z-10 w-full max-w-5xl flex flex-col items-center gap-5 transition-shadow duration-1000 ${ambientGlow}`}>
        ...
        <div className="w-full flex justify-between items-center bg-black/70 border border-emerald-500/25 backdrop-blur-md p-4 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.6)]">
          <div className="flex flex-col">
            <span className="text-[10px] text-emerald-500/50 font-bold tracking-[0.2em] uppercase">
              Agent ID
            </span>
            <span className="text-sm font-bold text-stone-100 tracking-wider uppercase">
              {socketId ? socketId.substring(0, 10) : "SYNCING..."}
            </span>
            <span className="text-xs text-emerald-400 mt-1 font-bold tracking-widest">
              SCORE <span className="text-emerald-300 text-base">{myScore}</span>
            </span>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-linear-to-r from-red-500 via-amber-400 to-red-500 drop-shadow-[0_0_12px_rgba(239,68,68,0.5)]">
              {roundDisplay.toString().padStart(2, "0")}
            </div>
          </div>

          <div className="flex flex-col items-end text-right">
            <span className="text-[10px] text-red-500/50 font-bold tracking-[0.2em] uppercase">
              Target
            </span>
            <span className="text-sm font-bold text-stone-100 tracking-wider uppercase">
              {opponentId.substring(0, 10)}
            </span>
            <span className="text-xs text-red-400 mt-1 font-bold tracking-widest">
              SCORE <span className="text-red-300 text-base">{opponentScore}</span>
            </span>
          </div>
        </div>
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <motion.div
            layout
            className={`relative flex flex-col items-center justify-center border-2 p-6 rounded-3xl h-72 md:h-80 transition-all duration-700 overflow-hidden ${
              myLocked
                ? "bg-emerald-950/30 border-emerald-400/70 shadow-[0_0_40px_rgba(16,185,129,0.25)]"
          ...
          >
            <div className="absolute top-4 left-4 flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-emerald-500/60 font-bold tracking-[0.25em] uppercase">
                Your Node
              </span>
            </div>
            ...
            <AnimatePresence mode="wait">
              {gameState !== "result" ? (
                ...
                >
                  <div
                    className={`w-24 h-24 rounded-full border-2 border-dashed flex items-center justify-center mx-auto mb-5 font-bold text-lg tracking-widest ${
                      myLocked
                        ? "border-emerald-400 text-emerald-400 shadow-[0_0_25px_rgba(16,185,129,0.4)]"
                  ...
                  <p className="text-xs text-stone-500 tracking-wide">
                    {myLocked ? "PAYLOAD SECURED" : "AWAITING SELECTION"}
                  </p>
                </motion.div>
              ) : (
                <motion.div
                ...
                >
                  {myChoice === "cooperate" ? (
                    <div className="text-emerald-400">
                      <Shield className="w-20 h-20 mx-auto mb-3 drop-shadow-[0_0_15px_rgba(16,185,129,0.6)]" />
                      <span className="text-2xl font-bold tracking-widest">COOPERATE</span>
                    </div>
                  ) : (
                    <div className="text-red-500">
                      <Zap className="w-20 h-20 mx-auto mb-3 drop-shadow-[0_0_15px_rgba(239,68,68,0.6)]" />
                      <span className="text-2xl font-bold tracking-widest">DEFECT</span>
                    </div>
                  )}
                </motion.div>
              )}
          ...
          <motion.div
            ...
            className={`relative flex flex-col items-center justify-center border-2 p-6 rounded-3xl h-72 md:h-80 transition-all duration-700 overflow-hidden ${
              opponentLocked
                ? "bg-red-950/20 border-red-500/50 shadow-[0_0_35px_rgba(239,68,68,0.15)]"
                : "bg-slate-900/40 border-slate-700/60"
            }`}
          >
            <div className="absolute top-4 left-4 flex items-center gap-1.5">
              <div className={`w-2 h-2 rounded-full ${opponentLocked ? "bg-red-500 animate-pulse" : "bg-slate-600"}`} />
              <span className="text-[10px] text-red-500/60 font-bold tracking-[0.25em] uppercase">
                Target Node
              </span>
            </div>
              ...
              {gameState !== "result" ? (
                ...
                >
                  <div
                    className={`w-24 h-24 rounded-full border-2 border-dashed flex items-center justify-center mx-auto mb-5 font-bold text-lg tracking-widest ${
                      opponentLocked
                        ? "border-red-500 text-red-500 shadow-[0_0_25px_rgba(239,68,68,0.35)] animate-pulse"
                        : "border-slate-600 text-slate-500"
                    }`}
                  >
                  ...
                  <p className="text-xs text-stone-500 tracking-wide">
                    {opponentLocked ? "OPPONENT LOCKED" : "OPPONENT DECIDING"}
                  </p>
                </motion.div>
              ) : (
                <motion.div
        ...
        <div className="w-full min-h-44 flex items-center justify-center bg-black/60 border border-slate-700/50 rounded-3xl p-5 backdrop-blur-md shadow-[0_0_40px_rgba(0,0,0,0.5)]">
          <AnimatePresence mode="wait">
            {gameState === "deciding" ? (
              ...
              >
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] text-red-500/50 tracking-widest uppercase">
                      Crack the firewall
                    </span>
                  </div>
                </button>
              ...
              >
                <div className="text-2xl md:text-3xl font-bold text-emerald-400 tracking-[0.35em] animate-pulse">
                  DECRYPTING CHOICES
                </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    {outcome === "dopamine" && (
                      <div>
                        <h3 className="text-2xl md:text-3xl font-bold text-amber-400 tracking-wide drop-shadow-[0_0_12px_rgba(245,158,11,0.5)]">
                          SUCCESSFUL HEIST
                        </h3>
                        <p className="text-xs text-amber-300/70 mt-1 tracking-wide">
                          You cracked their firewall while they trusted blindly. +300
                        </p>
                      </div>
                    )}
                    {outcome === "shame" && (
                      <div>
                        <h3 className="text-2xl md:text-3xl font-bold text-red-500 tracking-wide drop-shadow-[0_0_12px_rgba(239,68,68,0.5)]">
                          SYSTEM CORRUPTED
                        </h3>
                        <p className="text-xs text-red-400/70 mt-1 tracking-wide">
                          They stole your trust tokens. You got burned.
                        </p>
                      </div>
                    )}
                    {outcome === "hooray" && (
                      <div>
                        <h3 className="text-2xl md:text-3xl font-bold text-emerald-400 tracking-wide drop-shadow-[0_0_12px_rgba(16,185,129,0.5)]">
                          MUTUAL ALIGNMENT
                        </h3>
                        <p className="text-xs text-emerald-400/70 mt-1 tracking-wide">
                          Both agents preserved the matrix. Balanced split. +100
                        </p>
                      </div>
                    )}
                    {outcome === "ruin" && (
                      <div>
                        <h3 className="text-2xl md:text-3xl font-bold text-zinc-400 tracking-wide">
                          MUTUAL DESTRUCTION
                        </h3>
                        <p className="text-xs text-zinc-500 mt-1 tracking-wide">
                          Both chose greed. Firewalls annihilated. +50
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <button
                  onClick={resetNextRound}
                  className="shrink-0 px-6 py-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold tracking-wider flex items-center gap-2 transition-all duration-300 hover:shadow-[0_0_25px_rgba(16,185,129,0.5)] hover:-translate-y-0.5 active:translate-y-0"
                >
                  NEXT NODE
